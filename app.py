...
class YapayZekaDisTicaretDenetleyici:
    ...
    def mt700_ayristir(self, metin: str) -> dict[str, str]:
        ...
        for alan in hedef:
            desenler = [
                ...
                # Küşat/MT700 txt formatı: "46A:" veya "46A :" satır başında
                rf'(?:^|\n){re.escape(alan)}[ \t]*:[ \t]*(.*?)(?=\n[0-9]{{2,3}}[A-Z]?[ \t]*:|\Z)',
                # Alan adı + boşluk + değer (eski format)
                rf'(?:^|\n)[ \t]*{re.escape(alan)}[ \t]+(.+?)(?=\n[0-9]{{2,3}}[A-Z]?[ \t]|\n:|\Z)',
            ]
            if alan == "46A":
    ...
    def kilo_bul(self, metin: str) -> Optional[float]:
        """
        Weight parser — tüm formatları destekler:
        - Aynı satır: "Gross Weight: 3.420,00 KG"
        - Tablo başlık+sonraki satır: "Gross Weight (KG)\n3420"
        - TOTAL satırı: "TOTAL | 4.800 MTRS | 3.420,00 KG | ..."
        - Avrupa formatı: 3.420,00 → 3420.0
        """
        if not metin:
            return None
        m_u = metin.upper()
        ...
        desenler = [
            ...
            # Tablo TOTAL satırı: "TOTAL | 4.800,00 MTRS | 3.420,00 KG"
            r'TOTAL\s*\|[^|]+\|\s*([\d.,]+)\s*KG',
            # Tablo başlık + sonraki satır
            r'GROSS\s*WEIGHT[^\n]*\n\s*([\d.,]+)\s*(?:KGS?|MT\b|TON)?',
            ...
            # Satır başında: "3420 KG" veya "3.420,00 KG"
            r'^\s*([\d.,]+)\s*KGS?\s*$',
            r'([\d.,]+)\s*KGS?\b',
            r'([\d.,]+)\s*MT\b',
        ]
        for d in desenler:
            ...
            if m:
                v = normalize_tutar(m.group(1))
                if v and v > 0:
                    log.debug("[DEBUG] Kilo bulundu: %.2f (desen: %s)", v, d[:40])
                    return v
        return None
