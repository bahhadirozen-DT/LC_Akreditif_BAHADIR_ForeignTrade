...
# ---------------------------------------------------------------------------
try:
    from hukuk_motoru import ucp_kurallari_uygula, normalize_tutar as _norm_ext, mt700_hukuki_yorum
...
CO_46A_IFADELERI = [
    # Yalnızca LC'nin 46A alanında "CO belgesi isteniyor" anlamına gelen ifadeler
    "CERTIFICATE OF ORIGIN", "CERTIFICATE OF ORIGIN ISSUED BY",
    # KALDIRILDI: "CHAMBER OF COMMERCE" — fatura/küşat üzerinde geçince lc_co_istiyor=True yapıyordu
    # KALDIRILDI: "COUNTRY OF ORIGIN"   — faturada "Country of Origin: TURKIYE" geçince yanlış CO şartı tetikliyordu
]
...
class YapayZekaDisTicaretDenetleyici:
    ...
    def kilo_bul(self, metin: str) -> Optional[float]:
        """
        KUR 16: Weight parser — GROSS KG, NET KG, 3420 KGS varyantlarını destekler.
        """
        if not metin:
            return None
        desenler = [
            ...
            r'GROSS\s*[:\-]?\s*([\d,\.]+)\s*(?:KGS?|MT\b|TON)',  # "Gross: 3420 KGS"
        ...
        for d in desenler:
            ...
            if m:
                v = normalize_tutar(m.group(1))
                if v:
                    log.debug("[DEBUG] Kilo bulundu: %.2f (%s)", v, d[:30])
                    return v
        return None
    ...
    def origin_analizi_yap(self, kusat_text: str, fatura_text: str, alan_46a: str) -> dict:
        ...
        CO belgesi tespiti: DIGER_BELGELER içinde CERTIFICATE_OF_ORIGIN
        sınıfında olan ayrı bir dosyadan yapılır.
        Fatura içindeki "Country of Origin: TURKIYE" CO belgesi SAYILMAZ —
        sadece menşe bilgisi olarak raporlanır.

        Öncelik sırası (LC CO istiyorsa):
          1. Ayrı CO belgesi var → UYUMLU
          2. Fatura/PL/BL'de menşe beyanı var → UYUMLU (origin bilgisiyle)
          3. Hiçbir menşe bilgisi yok → MAJOR DISCREPANCY
        """
        ...
        # CO belgesi: yalnızca DIGER_BELGELER içindeki ayrı dosyadan tespit et
        # Fatura, Packing List, Konşimento içindeki "Certificate of Origin" ifadesi
        # CO belgesi anlamına GELMEZ.
        co_belgesi_var = False
        for d in (self.depo.get("DIGER_BELGELER") or []):
            if not isinstance(d, dict):
                continue
            metin_u = d.get("metin", "").upper()
            # Dosya başlığında "CERTIFICATE OF ORIGIN" açıkça geçmeli
            if "CERTIFICATE OF ORIGIN" in metin_u and "COMMERCIAL INVOICE" not in metin_u:
                co_belgesi_var = True
                break

        # Tüm belgelerden menşe bilgisi topla (Invoice, PL, BL dahil)
        tum_belgeler = {
            "Commercial Invoice":  fatura_text or "",
            "Packing List":        self._depo_metin("CEKI_LISTESI"),
            "Bill of Lading":      self._depo_metin("KONSIMENTO"),
        }
        origin_kaynak = None
        origin_ulke   = "TURKIYE"

        for kaynak_adi, metin in tum_belgeler.items():
            if not metin:
                continue
            if any(k in metin.upper() for k in ORIGIN_IFADELERI):
                origin_kaynak = kaynak_adi
                m = re.search(
        ...
        invoice_beyan = origin_kaynak is not None
        ...
        if not lc_co_istiyor:
            if invoice_beyan:
                return {
                    "durum": "BİLGİ",
                    "detay": f"Origin: {origin_ulke} | Kaynak: {origin_kaynak}",
                    "rezerv_gerekli": False,
                }
            return {
                "durum": "BİLGİ",
                "detay": "LC'de CO şartı tespit edilmedi.",
                "rezerv_gerekli": False,
            }
        ...
        if co_belgesi_var:
            return {
                "durum": "UYUMLU",
                "detay": "Certificate of Origin belgesi ibraz edildi.",
                "rezerv_gerekli": False,
            }
        ...
        if invoice_beyan:
            return {
                "durum": "UYUMLU",
                "detay": (
                    f"Origin: {origin_ulke} | Kaynak: {origin_kaynak} | "
                    "Ayrı CO belgesi ibraz edilmedi; fatura menşe beyanı kabul edildi."
                ),
                "rezerv_gerekli": False,
            }
        ...
        return {
            ...
            "detay": "LC CO istiyor ancak ne CO belgesi ne de herhangi bir belgede menşe beyanı bulunmadı.",
            "rezerv_gerekli": True,
        }
    ...
    def analiz_motoru(self) -> None:
        ...
        # round() ile karşılaştır — 26334.0 >= 26334.000000000004 precision fix
        _celiski_kontrolleri = [
            ("sigorta_eksik",
             round(sig_t, 2) if 'sig_t' in dir() and sig_t else None,
             round(fatura_tutar * 1.10, 2) if fatura_tutar else None),
        ]
