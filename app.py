...
class YapayZekaDisTicaretDenetleyici:
    ...
    def normalize_tutar(metin: str) -> Optional[float]:
        ...
        Avrupa (nokta=binlik, virgül=ondalık) ve Anglo (virgül=binlik, nokta=ondalık)
        formatlarının ikisini de destekler.
        """
        if not metin:
            return None
        # Para birimi sembollerini ve boşlukları temizle
        s = re.sub(r'[A-Za-z$€£\s]', '', metin).strip()
        if not s:
            return None
        try:
            # Format tespiti: son ayraç virgül mü nokta mı?
            son_virgul = s.rfind(',')
            son_nokta  = s.rfind('.')

            if son_virgul > son_nokta:
                # Avrupa formatı: 23.940,00
                s = s.replace('.', '').replace(',', '.')
            elif son_nokta > son_virgul:
                # Anglo formatı: 23,940.00
                s = s.replace(',', '')
            else:
                # Hiç ayraç yok: 23940
                pass
    ...
    def origin_analizi_yap(
        ...
        # Depoda Certificate of Origin belgesi var mı?
        co_belgesi_var = any(
            any(k in (d.get("metin", "").upper() if isinstance(d, dict) else d.upper())
                for k in CERTIFICATE_OF_ORIGIN_IFADELERI)
            for d in ([depo.get("DIGER_BELGELER") or []] +
                      [depo.get(k) for k in ["KUSAT","FATURA","KONSIMENTO","CEKI_LISTESI","SIGORTA"]
                       if depo.get(k)])
            if d
        )
        # Daha temiz: tüm depo metinlerini tara
        tum_metin = " ".join([
            self._depo_metin(k)
