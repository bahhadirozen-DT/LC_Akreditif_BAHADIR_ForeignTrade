...
# ---------------------------------------------------------------------------
try:
    from hukuk_motoru import ucp_kurallari_uygula, normalize_tutar as _norm_ext
...
class YapayZekaDisTicaretDenetleyici:
    ...
    def origin_analizi_yap(self, kusat_text: str, fatura_text: str, alan_46a: str) -> dict:
        ...
        'Country of Origin: TURKIYE' ve 'goods are of Turkish origin' tanınır.
        CO belgesi yoksa 'Certificate of Origin mevcut' YAZILMAZ.
        """
        ...
        if not lc_co_istiyor:
            # LC CO şartı yok — faturada origin bilgisi varsa raporla
            if invoice_beyan:
                return {
                    "durum": "BİLGİ",
                    "detay": "Origin bilgisi fatura üzerinde bulundu. Ayrı Certificate of Origin belgesi sunulmadı.",
        ...
        if invoice_beyan:
            return {
                "durum": "UYARI",
                "detay": (
                    "Origin bilgisi fatura üzerinde bulundu. "
                    "Ayrı Certificate of Origin belgesi sunulmadı. "
                    "Bu ifade CO YERİNE GEÇMEZ (ISBP 821)."
                ),
                "rezerv_gerekli": True,
            }
        ...
        if HUKUK_MOTORU_AKTIF and ucp_kurallari_uygula:
            try:
                ...
                if isinstance(ucp_sonuc, list) and ucp_sonuc:
                    ...
                    log.debug("[DEBUG] UCP tablosu: %d kayıt", len(ucp_sonuc))
            except Exception as e:
