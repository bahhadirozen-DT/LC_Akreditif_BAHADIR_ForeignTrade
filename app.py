...
class YapayZekaDisTicaretDenetleyici:
    ...
    def origin_analizi_yap(self, kusat_text: str, fatura_text: str, alan_46a: str) -> dict:
        ...
        # KUR 8: Invoice'daki Türkçe menşe beyanı CO sayılmaz
        ...
        if invoice_beyan:
            return {
                "durum": "UYARI",
                "detay": (
                    "Invoice'da menşe beyanı mevcut ('goods are of Turkish origin') "
                    "ancak bu ifade Certificate of Origin YERİNE GEÇMEZ (KUR 8 / ISBP 821). "
                    "Ayrı CO belgesi ibraz edilmeli."
                ),
                "rezerv_gerekli": True,
            }
        return {"durum": "MAJOR DISCREPANCY",
                "detay": "LC CO istiyor ancak ne CO belgesi ne de invoice beyanı bulunmadı.",
                "rezerv_gerekli": True}
    ...
    def rapor_kaydet(self) -> None:
        """
        BUG-06 FİX: Tek içerik üretilir (markdown_raporu()),
        hem dosyaya yazılır hem konsola yazdırılır.
        """
...
<h2>SWIFT Rezerv Simülatörü</h2>{swift_html}
    ...
    def baslat(self) -> None:
        ...
        if self.depoyu_tara():
            ...
            # BUG-01 FİX: Tek analiz motoru
            self.analiz_motoru()
            # BUG-06 FİX: Tüm çıktılar aynı analysis_result'tan
            self.rapor_kaydet()
            self.html_raporu()
        else:
