...
Düzeltilen Kritik Hatalar:
  ...
  - BUG-02: re.search naive digit pattern ilk rakamı alıyordu (örn. Invoice No)
...
# ---------------------------------------------------------------------------
# Tutar normalleştirme — BUG-01 FIX (aynı mantık app.py ile senkron)
# ---------------------------------------------------------------------------
def _normalize_tutar(metin: str) -> Optional[float]:
    ...
    try:
        ...
        if virgul_say == 0 and nokta_say == 0:
            return float(s)
        elif virgul_say == 1 and nokta_say == 0:
            ondalik = s[son_virgul+1:]
            s = s.replace(',', '') if len(ondalik) == 3 else s.replace(',', '.')
        elif nokta_say == 1 and virgul_say == 0:
            ondalik = s[son_nokta+1:]
            if len(ondalik) == 3:
                s = s.replace('.', '')
...
# ---------------------------------------------------------------------------
# CIF fatura ayrıştırıcı — BUG-01 FIX
# ---------------------------------------------------------------------------
def _invoice_tutarlari_ayristir(metin: str) -> dict[str, Optional[float]]:
    def _bul(desenler: list[str]) -> Optional[float]:
        for d in desenler:
            m = re.search(d, metin, re.IGNORECASE)
            if m:
                v = _normalize_tutar(m.group(1))
                if v is not None:
                    return v
        return None
...
def _lc_karsilastirma_tutari(d: dict) -> Optional[float]:
...
def _kilo_bul(metin: str) -> Optional[float]:
    ...
    Naive digit-only pattern kullanılmaz.
    """
    if not metin:
        return None
    desenler = [
...
def analiz_et(depo: dict[str, Any]) -> list[tuple[str, str, str, str]]:
    ...
    # ── 4. Tutar Çapraz Kontrolü — BUG-01 FIX: CIF toplam kullan ──────────
    kusat_text = _metin(depo, "KUSAT")

    if fatura_text and kusat_text:
        try:
            fat_tutarlari = _invoice_tutarlari_ayristir(fatura_text)
            fat_tutar     = _lc_karsilastirma_tutari(fat_tutarlari)

            lc_32b = re.search(r':?32B[:\s]*[A-Z]{3}\s*([\d,\.]+)', kusat_text, re.IGNORECASE)
            lc_tutar = _normalize_tutar(lc_32b.group(1)) if lc_32b else None
            if lc_tutar is None:
                lc_tutar = _lc_karsilastirma_tutari(_invoice_tutarlari_ayristir(kusat_text))

            log.debug("Tutar karşılaştırma: Fatura CIF=%s | LC=%s", fat_tutar, lc_tutar)
            ...
            if fat_tutar is not None and lc_tutar is not None and lc_tutar > 0:
                ...
                if sapma <= tolerans:
                    rapor.append(("Art 30", "Tutar Tolerans Kontrolü", "OK",
                                  f"Fatura CIF: {fat_tutar:,.2f} | LC: {lc_tutar:,.2f} | "
